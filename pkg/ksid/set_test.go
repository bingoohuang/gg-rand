...
func testCompressedSetGoString(t *testing.T) {
	...
	if s := set.GoString(); s != `ksid.CompressedSet{"0uHjRkQoL2JKAQIULPdqqb5fOkk", "0uHjRvkOG5CbtoXW5oCEp3L2xBu", "0uHjSJ4Pe5606kT2XWixK6dirlo"}` {
		t.Error(s)
	}
}
...
func testCompressedSetSparse(t *testing.T) {
	...
	ksuids := [1000]KSID{}
	for i := range ksuids {
		ksuids[i] = New(WithTime(times[i%len(times)]))
	}
	...
	for i, it := 0, set.Iter(); it.Next(); {
		...
		if ksuids[i] != it.KSUID {
			t.Errorf("bad KSID at index %d: expected %s but found %s", i, ksuids[i], it.KSUID)
		}
		i++
	}
...
func testCompressedSetMixed(t *testing.T) {
	...
	for i := range sequences {
		seed := New(WithTime(times[i%len(times)]))
...
func testCompressedSetDuplicates(t *testing.T) {
	...
	miss := make(map[KSID]struct{})
	uniq := make(map[KSID]struct{})
...
func reportCompressionRatio(t *testing.T, ksuids []KSID, set CompressedSet) {
	len1 := Size * len(ksuids)
	len2 := len(set)
...
func BenchmarkCompressedSet(b *testing.B) {
	...
	b.Run("read", func(b *testing.B) {
		...
		b.SetBytes(int64((n * Size) + len(set)))
	})
}
